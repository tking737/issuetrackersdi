export async function sendResolvedEmails(options: {
  ...
  const recipients = Array.from(new Set(options.recipients.map((value) => value.trim().toLowerCase()).filter(Boolean)));
    ...
    const mailRes = await fetch(`https://graph.microsoft.com/v1.0/users/${encodeURIComponent(sender)}/sendMail`, {
      ...
      body: JSON.stringify({
        message: {
          ...
          body: {
            contentType: "HTML",
            content: `<p>Your issue <strong>${options.issueTitle}</strong> has been marked as resolved.</p><p><a href="${options.appUrl}/?issue=${options.issueId}">Open the ticket</a></p>`,
          },
